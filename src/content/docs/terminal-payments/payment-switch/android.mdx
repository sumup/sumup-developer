...
Payment Switch is no longer actively being developed. Prefer the [Android Reader SDK](/terminal-payments/sdks/android-sdk/) for native mobile integrations, and use Payment Switch only when the native SDK and the [Cloud API](/terminal-payments/cloud-api/) are not viable for your setup.
