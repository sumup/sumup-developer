...
1. Using the OAuth2 [flow](https://developer.sumup.com/tools/authorization/authorization#integration-via-oauth-20):
...
2. Using API Key:
   You can use an API Key as an auth token. Generate the key in the [SumUp Dashboard](https://developer.sumup.com/tools/authorization/authorization#integration-via-api-keys) and provide it to the SDK through `AuthTokenProvider.getAccessToken()` method.
