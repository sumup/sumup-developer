...
import { androidttpsdkname } from "@lib/productnames.ts";
import { Aside } from "@astrojs/starlight/components";
import AndroidTapToPayInstallSnippets from "@components/content/AndroidTapToPayInstallSnippets.astro";

The { androidttpsdkname } enables your mobile app to accept card-present contactless payments using only a smartphone, without any additional hardware. This guide explains how to integrate this functionality into your Android app.
...
- For the latest updates, consult the [{ androidttpsdkname } Changelog](/changelog/android-tap-to-pay-sdk).
...
- Access to the { androidttpsdkname } GitHub repository (request by emailing integration@sumup.com).
...
2. Add the dependency to a module `build.gradle`:
   ...
   <AndroidTapToPayInstallSnippets variant="production" />

   Testing dependency:

   <AndroidTapToPayInstallSnippets variant="testing" />

### Authentication

The { androidttpsdkname } uses a transparent authentication approach: authentication is handled by the consumer app, not the SDK.
...
The { androidttpsdkname } may return a `Result.Failure` containing an exception when one of its methods is called.
...
To test the { androidttpsdkname } in Brazil and Chile, you should set `processCardAs` for each transaction. For example:
