...
description: Integrate with Android Tap-to-Pay SDK to process contactless payments on a phone.
sidebar:
  order: 15
---
...
## About Android Tap-to-Pay SDK

{ androidttpsdkname } enables your mobile app to accept card-present contactless payments with a smartphone only, without the need for any additional hardware.
...
- For latest information, please check the [{ androidttpsdkname } Changelog](/changelog/tap-to-pay).
...
- Access to { androidttpsdkname } GitHub repository (please send an email to integration@sumup.com).
...
{ androidttpsdkname } uses the transparent authentication approach.
...
{ androidttpsdkname } may return a `Result.Failure` containing an exception when one of its methods is called.
...
To test { androidttpsdkname } in Brazil and Chile, you should set `processCardAs` for each transaction. For example:
...
Additionally, each transaction in Brazil and Chile should be verified by PIN.
