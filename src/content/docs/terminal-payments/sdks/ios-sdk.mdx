...
:::note
Availability is determined at runtime by SumUp, not by a list you can check at build time, so treat `checkTapToPayAvailability` as the only source of truth. In Chile the feature additionally requires iOS 18 or later, which is higher than the general minimum.
