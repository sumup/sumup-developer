---
title: Cloud API
description: Details how to use Solo card readers and run remote in-person payments via the Cloud API.
sidebar:
  order: 2
---
...
## About Payments with { cloudapiname }

The { cloudapiname } allows you to start a transaction from a Point of Sales (POS) running on any platform (Windows, iOS, Linux, Android, Web-based etc.) capable of sending HTTPS requests and complete said transaction via a Solo reader.
...
* No distance limitation between your POS device and the Solo card reader. You can
even send transactions remotely.
...
* Your device must be authorized to use the { cloudapiname }. [Read the authorization guide](/tools/authorization/authorization/) and implement a method that best fits your use case. The API key should be sufficient if you do not intend to delegate access to third parties.
* You need to [create an Affiliate Key](/tools/authorization/affiliate-keys/) for your app. SumUp { cloudapiname } requires the Affiliate Key to be present in checkout requests.
* We strongly recommend keeping the Solo terminal plugged-in when using { cloudapiname }.
...
If you don't have the Solo reader yet, you can still try out the flow using the [Virtual Solo](https://virtual-solo.sumup.com) with a [SumUp test account](/online-payments/#getting-a-test-account)!
...
## Pairing Solo Reader via { cloudapiname }

The { cloudapiname } works as bridge between your application and the Solo reader. The { cloudapiname } has two key processes:
...
{ cloudapiname } pairing process is only available for logged-out users. If you're logged in to your merchant account, do as follows:
...
merchant account. This enrollment process is what we call reader pairing.
The pairing process starts on the card reader side by generating the pairing code, and finishes
when the target device successfully completes a pairing request using the { cloudapiname }, providing the code.
...
{ cloudapiname } provides the possibility to manage transactions using the connected reader(s), as well as reader management endpoints. For details, check the section below. 
