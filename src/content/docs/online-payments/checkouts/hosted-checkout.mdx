...
<Callout type="note">
  A Hosted Checkout session is available for 30 minutes. After that, unpaid
  sessions show an expired or not found page.
</Callout>
  ...
  options={[
    {
      label: "cURL",
      lang: "bash",
      code: String.raw`curl -X POST https://api.sumup.com/v0.1/checkouts \
      ...
      code: String.raw`var checkout = await client.Checkouts.CreateAsync(new CheckoutCreateRequest
{
    ...
    },
    {
      label: "Go",
      lang: "go",
      code: String.raw`checkout, err := client.Checkouts.Create(ctx, sumup.CheckoutsCreateParams{
    MerchantCode:      merchantCode,
    Amount:            12.0,
    ...
    },
    {
      label: "Python",
      lang: "py",
      code: String.raw`checkout = client.checkouts.create(
    CreateCheckoutBody(
    ...
    },
    {
      label: "Rust",
      lang: "rust",
      code: String.raw`let checkout = client.checkouts().create(Some(CheckoutCreateRequest {
    merchant_code,
    amount: 12.,
    currency: Currency::EUR,
...
<Callout type="tip">
  Treat the hosted page as the customer interface, but use the checkout status
  and webhook events as the source of truth for your backend order state.
</Callout>
