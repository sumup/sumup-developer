...
| Direct Checkouts API                                           | Your client must follow the returned `next_step` instructions. This path requires PCI compliance and explicit onboarding and approval from SumUp.                       |

Hosted Checkout or the Payment Widget is the recommended path unless you have been approved for a direct Checkouts API integration.

## How the payment flow works

1. Your backend [creates a checkout](/api/checkouts/create) with a unique `checkout_reference` and a `redirect_url`.
2. The customer submits their payment through Hosted Checkout, the Payment Widget, or an approved direct integration.
...
### Handle `next_step` in a direct integration

When more customer action is required, processing a checkout can return a `next_step` object:

```json
{
  "next_step": {
    "method": "POST",
    "url": "https://issuer.example/authenticate",
    "redirect_url": "https://merchant.example/payments/complete",
    "mechanism": ["iframe", "browser"],
    "payload": {
      "provider_defined_key": "provider-defined-value"
    }
  }
}
```
...
1. Use the HTTP method in `next_step.method`.
2. Submit the request to the exact URL in `next_step.url`.
3. Include every key-value pair from `next_step.payload`. The keys vary by provider and transaction, so do not hard-code or rename them.
4. Use one of the presentation options returned in `next_step.mechanism`.
5. After the customer returns, ask your backend to retrieve the checkout and confirm its final status.

Do not expose your SumUp API key to the client or log the full `next_step.payload`.
