...
1. [Create a checkout](/api/checkouts/create#create-a-checkout)
...
6. Submitting the payment sheet triggers the `onpaymentauthorized` callback. At that point, [process the checkout](/api/checkouts/process#process-a-checkout). The process-checkout request body needs to include a `payment_type` of `apple_pay` and an `apple_pay` object containing the Apple Pay payment token returned by the callback.
...
<Callout type="tip">
  ...
  <a href="/api/">API contract & guidelines</a>
</Callout>
