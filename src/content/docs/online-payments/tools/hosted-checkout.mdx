...
All [SumUp supported payment methods](/online-payments/apm/) are available in Hosted Checkout, according to availability per merchant.

<image
  alt="Hosted Checkout Form"
  src="/img/guides/hosted_checkout_landing_page.png"
  width="80%"
  style="display: block; margin-left: auto; margin-right: auto; border: 1px solid var(--cui-border-normal)"
/>
...
The only requirement to access this product is the ability to create online checkouts ([see more](/api/checkouts/create)).

## How does it work?

Once you're able to create checkouts, you can opt-in for Hosted Checkout by providing an extra parameter to your POST request payload.
...
- `hosted_checkout` will be returned as provided in the request
...
The response body containing the `hosted_checkout` and `hosted_checkout_url` parameters:
...
<Aside type="note">
  Each Hosted Checkout session lasts for 30 minutes, after which end users will
  be presented with a not found page upon access attempts.
</Aside>
...
This product is meant to suit a broad range of use cases, from a simple self-contained payment solution, to a checkout step integrated within a longer flow.
When creating a checkout, it's possible to provide a `redirect_url` parameter to include a redirect link in the success page.

<Aside type="note">
  Differently from what's stated in our [API docs](/api/checkouts/create), the
  absence of this parameter does not affect the availability of payment methods
  in Hosted Checkouts.
</Aside>
...
<Aside type="caution">
This product is currently in beta and may be subject to change and we're currently working on many new features soon to be available!
Keep an eye on this page as we'll continue to add features in the upcoming months.
