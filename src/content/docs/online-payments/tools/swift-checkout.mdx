---
title: Swift Checkout Implementation
---
...
<Aside type="caution">
  For pages that have a strict Content Security Policy (CSP) visit the [Handling
  CSP](/online-payments/tools/card-widget/#handling-strict-content-security-policies)
  instructions.
</Aside>
...
<Aside type="note">
  Please note, that Swift Checkout SDK is in Alpha version, expect changes to be
  communicated in the future. For any feedbacks or questions, you can reach out
  to us through our [contact form](/contact).
</Aside>
...
<Aside type="note">
  The `abort` method should only be used if the payment request has not yet been
  processed. Attempting to cancel a payment request after it has been processed
...
<Aside type="note">
  You can create your own custom buttons, but we recommend using the built-in
  PaymentElement UI builder.
</Aside>
