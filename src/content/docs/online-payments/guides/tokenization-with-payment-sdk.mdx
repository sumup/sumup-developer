...
description: Shows how to tokenize cards with the Payment Widget so you can save payment instruments for future charges.
sidebar:
  order: 200
---
...
4. [Make subsequent payments with the tokenized card](#processing-recurring-payments).
...
It contains personal information such as their name, contact details, postal address, as well as a unique identifier that is relevant to your business logic (`customer_id`).
...
<Tabs syncKey="backend_lang">
  ...
  <TabItem label="Go" icon="seti:go">
    ```go
    customerID := "MYCUSTOMERID-123"
    purpose := sumup.CreateCheckoutBodyPurposeSetupRecurringPayment
    description := "My checkout"
  ...
  <TabItem label="PHP" icon="seti:php">
    ...
    $response = $customService->request('POST', '/v0.1/checkouts', $payload);
    ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="backend_lang">
  ...
  <TabItem label="PHP" icon="seti:php">
    ```php
    $customersService = $sumup->getCustomerService();
    $response = $customersService->getPaymentInstruments('MYCUSTOMERID-123');
    ```
  </TabItem>
</Tabs>
...
If you have successfully processed the checkout, a token representing the payment instrument (card) is created. You can now [retrieve the checkout](/api/checkouts/get) to find the token within a `payment_instrument` object for later recurrent payment.
