...
The [SumUp Payment Widget](/online-payments/checkouts/card-widget/) is a JavaScript library that securely collects card details and processes checkouts while handling consent collection and 3D Secure authentication. We recommend the Payment Widget because it handles these steps within the embedded checkout.

Once you have a checkout reference from the step above, you can mount the Payment Widget on your website and collect the card details. Pass the `checkout_reference` from above as the `checkoutId` here.
