...
- You have a valid access token obtained via the [Authorization code flow](/online-payments/authorization/#authorization-code-flow).
- You have processed successfully a payment checkout and you need to refund the transaction.

<Aside type="note">

Transactions are associated with active merchant user accounts. As a result, you _cannot_ use an access token obtained via the [Client credentials flow](/online-payments/authorization/#client-credentials-flow) to complete the steps in this guide.
