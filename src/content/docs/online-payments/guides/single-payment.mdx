---
title: Make a payment with a card entered by a customer
sidebar:
...
- You have [registered your client application](/online-payments/introduction/register-app/) with SumUp.
- You have a valid access token obtained via one of the supported OAuth2 authorization flow, the [Authorization code flow](/online-payments/authorization/#authorization-code-flow) or [Client credentials flow](/online-payments/authorization/#client-credentials-flow).

<Aside type="note">

You can create and complete a payment checkout for any valid SumUp merchant user's account. As a result, you _can_ use an access token obtained either via the [Client credentials flow](/online-payments/authorization/#client-credentials-flow) or via the [Authorization code flow](/online-payments/authorization/#authorization-code-flow).
