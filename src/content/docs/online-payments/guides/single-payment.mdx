...
description: Teaches how to create and complete a checkout for a one-off card payments.
sidebar:
  order: 150
---
...
The purpose of this guide is to explain processing a single payment over SumUp API, in case you don't want to use our [Payment Widget](/online-payments/checkouts/card-widget). In this guide, you will learn how to make a single payment with a payment card entered by a customer without associating it with a specific customer saved for the merchant user's account. This type of payment is most useful for quick processing of payment checkouts in which you don't store and manage any account data for your customers.
...
You can create and complete a payment checkout for any valid SumUp merchant user's account. As a result, you _can_ use an access token obtained either via the [Client credentials flow](/tools/authorization/authorization/#client-credentials-flow) or via the [Authorization code flow](/tools/authorization/authorization/#authorization-code-flow).
