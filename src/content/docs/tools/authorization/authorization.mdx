...
SumUp supports two kinds of authorization. One is via [API keys](#api-keys) allowing the app to authenticate and authorize on behalf of the Merchant and access the SumUp API with full set of permissions.
...
Should you decide to integrate third-parties with the API key, you are responsible for ensuring the security of such integration.
...
OAuth 2.0 is the recommended approach for building advanced integrations, where multiple different merchants or their employees want to use SumUp.
...
The `access_token` can now be used in the `Authorization` header with `Bearer` scheme to authorize calls to SumUp services.
