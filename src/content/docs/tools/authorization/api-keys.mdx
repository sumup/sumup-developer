...
API keys are static credentials that let your application act on behalf of the merchant account that created the key.
Use them when you control the merchant account and need direct access to SumUp APIs with the full set of permissions.
...
Do not expose secret API keys in publicly accessible places such as client-side code or repositories.
API keys grant broad access to the merchant account, so treat them as sensitive credentials.
...
4. Select **API Keys**.
   ...
   If this is your first key, the page only shows the SumUp Public Key.
   Existing API keys are also listed here.
