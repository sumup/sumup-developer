...
OAuth 2.0 lets your application request permission to act on behalf of other SumUp merchants or their employees.
It follows the industry-standard [OAuth 2.0 protocol](https://oauth.net/2/) and is the right choice for multi-merchant integrations or when you cannot rely on a single merchant-owned API key.
...
Access tokens expire, for example when a user changes their password or revokes access.
...
To integrate an external application with SumUp, register an OAuth application and generate client credentials.
These credentials let you complete the OAuth flows in this guide and obtain access tokens for protected SumUp resources.
...
- A SumUp merchant account with completed [account details](https://me.sumup.com/account). Reach out through the [contact form](/contact) if you need a test account.
...
Log in to [your SumUp account](/auth/login). Once logged in, **Account** appears in place of the **Log in** button at the top right.

### 2. Create an OAuth application

Navigate to the [OAuth Apps](/apps/) page to create or manage OAuth applications.
...
On the [OAuth Apps](/apps) page, open your application and choose **Create client secret**.
