...
You should not rely on the information here, unless you are building a feature
...
present. Fields not supported in a country will be ignored. For example:
...
The Merchants API uses a new Person model, which is exposed as a sub-resource of
the merchant. A Person is someone who plays some legally relevant role in the
Merchant. This can be what we call legal representative, a business owner, or a
business officer.
...
- **Legal Representative**: The person legally representing the business
...
- **Hong Kong** (`HK`)
  ...
  - **Hong Kong Identity Card** `hk.hkid`\
    TODO
...
- **Malaysia** (`MY`)
  ...
  - **National Registration Identity Card** `my.nric`\
    No description available
